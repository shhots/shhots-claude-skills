...
**Claude.ai (web, desktop, mobile):** download the skill's `.zip` from [Releases](../../releases) and upload it at Settings > Skills. These ZIPs use condensed descriptions to fit claude.ai's 200-character limit; the plugin versions above keep the full trigger phrasing. To build the ZIPs yourself, run `scripts/build-skill-zips.sh` (output in `dist/`).
